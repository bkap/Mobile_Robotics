...
void cameraCallback(const sensor_msgs::Image::ConstPtr& msg){
	sensor_msgs
	cv::Mat image;
	sensor_msgs::PointCloud keypoints = findKeypoints(image);
	tfl->transformPointCloud("map",keypoints,last_map_cloud);
	P.publish(last_map_cloud);
}

sensor_msgs::PointCloud findKeypoints(cv::Mat& image){

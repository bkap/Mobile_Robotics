...
class DemoNode {
  public:
    ...
    void publishBlobLoc(Point32 BlobLoc);
  ...
  private:
    ...
    PointCloud BlobLocations;
};

//call this with a point32 to publish the blob's location
void DemoNode::publishBlobLoc(Point32 BlobLoc)
{
   BlobLocations.pts[0] = BlobLoc;
   pub_blob_loc.publish(BlobLocations);
}
DemoNode::DemoNode():
  it_(nh_)
{
  ...
  pub_blob_loc = nh_.advertise<geometry_msgs::PointCloud>("Cam_Cloud", 1);
  BlobLocations.width = 0;
  BlobLocations.height = 0;
  BlobLocations.pts = (Point32*) malloc(1*sizeof(Point32));
}
